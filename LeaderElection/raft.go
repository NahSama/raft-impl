...
entry notifies the client that consensus was reached on a command, so it can
...
type ConsensusModule struct {
	// mutex to protect concurrent access to the CM
	mu sync.Mutex

	// server Id of this CM
	id int

	// list of ids of peers in the cluster
	peerIds []int

	// server is the server contains this CM. It is used to issue RPC calls
	server *Server
	...
	// commit new entries to the log to notify that entries may be sent to commitChan
	newCommitReadyChan chan struct{}

	// Persistence raft state on all servers
	currentTerm int
	votedFor    int
	log         []LogEntry
...
func (cm *ConsensusModule) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	cm.dlog("Request Vote: %+v [currentTerm=%d, votedFor=%d]", args, cm.currentTerm, cm.votedFor)
	...
	if cm.currentTerm == args.Term && (cm.votedFor == -1 || cm.votedFor == args.CandidateId) {
		reply.VotedGrant = true
		cm.votedFor = args.CandidateId
		cm.electionResetEvent = time.Now()
	} else {
		reply.VotedGrant = false
	}
...
func (cm *ConsensusModule) startElection() {
	...
	// send RequestVote to other servers concurrently
...
// becomeFollower makes cm a follower and reset its state
...
func (cm *ConsensusModule) startLeader() {
	cm.state = Leader
	cm.dlog("become Leader; term=%d, log=%v", cm.currentTerm, cm.log)
